    ...
    def create_zuul_jwt(self, user):
        ...
        if user['login'] not in ['admin', 'SF_SERVICE_USER']:
            ...
            for project in projects:
                source_repos = projects[project]['source-repositories']
                source_repos_names = sum((x.keys() for x in source_repos),
                                         [])
                canonical_repos_names = [
                    orig_conf.zuul['default_canonical_name'] + '/' + r
                    for r in source_repos_names]
                tenant_name = projects[project].get(
                    'tenant',
                    orig_conf.zuul.get('default_tenant', 'local'))
        ...
        try:
            token = jwt.encode(
                ...
                algorithm=orig_conf.zuul['JWTalgorithm']).decode('utf-8')
