        ...
        if allowed_orgs:
            ...
            if not allowed:
                self.twarning("Organization %s are not allowed",
                              transactionID, str(set(user_orgs)))
                return False
        return True
...
class GithubPersonalAccessTokenAuthPlugin(BaseGithubAuthPlugin):
    ...
    def get_user_orgs(self, token, transactionID):
        basic_auth = requests.auth.HTTPBasicAuth(token,
                                                 'x-oauth-basic')
        return requests.get("https://api.github.com/user/orgs",
                            auth=basic_auth)
    ...
    def authenticate(self, **auth_context):
        ...
        self.tdebug('Attempting basic auth against github server',
                    transactionID)
        resp = requests.get("https://api.github.com/user", auth=basic_auth)
        if not resp.ok:
        ...
        if self.read_ssh_keys:
            ...
            if not resp.ok:
                ...
                raise base.UnauthenticatedError("Failed to retrieve ssh keys")
